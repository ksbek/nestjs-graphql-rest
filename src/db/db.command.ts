...
export class DbCommand {
  ...
  @Command({ command: 'db:seed:languages', describe: 'populate database with static collections', autoExit: true })
  async dbSeed() : Promise<any> {
    await this.dbFixturesService.loadFixtures();
  }

  @Command({ command: 'db:seed:developers', describe: 'populate database with fake developers', autoExit: true })
  async dbSeedDevelopers() : Promise<any> {
    await this.dbFixturesService.loadDevelopers();
  }

  @Command({ command: 'db:seed:drop', describe: 'drop static collections', autoExit: true })
